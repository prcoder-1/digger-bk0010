...
uint8_t  bug_snd_c1;         /// Счётчик нот внутри фазы звука съедения врага (20..0)
uint8_t  bug_snd_c2;         /// Счётчик фаз звука съедения врага (4..0)
uint16_t bug_snd_period;     /// Текущий затухающий период звука съедения врага
uint16_t bug_snd_period_held; /// «Удерживаемое» значение периода (звучит 2 ноты подряд)
...
static void sound_effect()
{
    ...
    if (snd.bug) // Звук съедаемого врага - партиями по 10 нот за вызов (фоновое воспроизведение)
    {
        // 3 фазы по 20 нот = 60 нот всего. Партии по 10 → ~7 кадров суммарно,
        // ~30 мс блокировки на кадр (вписывается в 100 мс при 10 FPS).
        for (uint8_t sounds = 10; sounds && snd.bug; )
        {
            if (bug_snd_c1)
            {
                uint8_t c1_lb = bug_snd_c1 & 3;
                if (c1_lb == 0) bug_snd_period_held = bug_snd_period;
                if (c1_lb == 2) bug_snd_period_held = bug_snd_period - (bug_snd_period >> 4);
                bug_snd_c1--;
                bug_snd_period -= bug_snd_period >> 4;
                sound(bug_snd_period_held / 16, 25);
                sounds--;
            }
            else
            {
                bug_snd_c2--;
                if (!bug_snd_c2) { snd.bug = 0; break; } // Четвёртое срабатывание - завершить
                bug_snd_period = 1600;
                bug_snd_c1 = 20;
            }
        }
        ...
        else
        {
            ...
            for (uint8_t i = 0; i < bugs_max; ++i)
            {
                ...
                if (bonus_state == BONUS_ON)
                {
                    ...
                    // Запустить фоновое воспроизведение звука съедения врага (3 фазы по 20 нот)
                    snd.bug = 1;
                    bug_snd_c1 = 0;
                    bug_snd_c2 = 4;
                    bug_snd_period = 0;
