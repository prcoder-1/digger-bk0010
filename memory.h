...
union KEY_STATE
{
    volatile struct
    {
        uint16_t UNUSED_1 : 6; /**< Не используются */
        uint16_t INT_MASK : 1; /**< Маска прерываний от клавиатуры */
        uint16_t STATE    : 1; /**< Флаг состояния клавиатуры */
        uint16_t UNUSED_2 : 8; /**< Не используются */
    } bits;
...
/**
 * @brief Регистр данных клавиатуры
 */
union KEY_DATA
{
    volatile struct
    {
        uint16_t CODE   : 7; /**< Семиразрядный код клавиши (только для чтения) */
        uint16_t UNUSED : 9; /**< Не используются */
    } bits;
...
union TVE_CSR
{
    volatile struct
    {
        uint16_t SP     : 1; /**< Выбор режим тактирования счетчика таймера по ниспадающему фронту на входе nSP */
        uint16_t CAP    : 1; /**< Режим захвата. При нулевом значении бита CAP таймер генерирует
                              * событие при переходе счетчика через значение нуль и осуществляет
                              * загрузку регистра счетчика из регистра предела. */
        uint16_t MON    : 1; /**< Разрешить мониторинг события таймера */
        uint16_t OS     : 1; /**< Использовать однократный режим, при достижении счетчиком нуля,
                              * бит RUN будет сброшен, таймер перезагружен и остановлен */
        uint16_t RUN    : 1; /**< Запись единицы разрешает работу счетчика */
        uint16_t D16    : 1; /**< Разрешить делитель частоты на 16 */
        uint16_t D4     : 1; /**< Разрешить делитель частоты на 4 */
        uint16_t FL     : 1; /**< Флаг события таймера */
        uint16_t unused : 8; /**< Не используется */
    } bits;
