...
static inline union SSD EMT_34()
{
    union SSD rv;
    asm volatile (
        // Ведущий ноль ОБЯЗАТЕЛЕН: для GNU as «34» — десятичное (= emt 042,
        // передача байта в ТЛГ-линию!), а не октальное 034. Из-за этого чтение
        // ССД возвращало мусор (а на живом БК могло зависнуть в ожидании линии).
        "emt 034\n"
        "mov r0, %0"
        : "=r" (rv.reg) : : "r0", "cc"
    );
