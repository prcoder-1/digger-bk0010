...
static void print_credits()
{
    ...
    while (*p)
    {
        // Длина строки в ЗНАКОМЕСТАХ (для центрирования). В строку могут быть
        // встроены управляющие коды драйвера (цвет, 0220..0237) — курсор они не
        // двигают, поэтому в ширину не считаются.
        const char *e = p;
        uint8_t len = 0;
        while (*e && *e != '\n')
        {
        ...
        // Центрированное начало строки: столбец 0 (рамка) не трогаем, при len<=15
        // столбец >= 1. Позиционируем курсор и печатаем строку символ за символом
        // (включая встроенные коды цвета).
