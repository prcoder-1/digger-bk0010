...
/**
 * @brief Сброс состояния демо в исходное
 *
 * Все переменные состояния демо лежат в .data и переживают перезапуск заставки
 * по клавише «СТОП» (вектор 4 указывает на _start, образ заново не грузится),
 * поэтому сбрасываем их явно, а не полагаемся на статические инициализаторы.
 * Без этого демо продолжалось с момента останова: ненулевой demo_time не давал
 * отработать блоку `demo_time == 0` (единственное место вывода левой панели),
 * а актёры рисовались по старым координатам вне своего слота таймлайна.
 */
static void reset_demo_state()
{
    demo_time = 0;
    nobbin_x = nobbin_y = hobbin_x = hobbin_y = digger_x = digger_y = 0;
    hobbin_mirror = digger_mirror = false;
    image_phase = 0;
    image_phase_inc = 1;
    panel_keys = false; // Первым циклом показываем кредиты
}
...
/**
 * @brief Приведение дисплея в известное состояние: цветной режим (32 символа
 *        в строке = широкий шрифт), зелёный текст, погашенный курсор
 *
 * Коды 0233 и 0232 ПЕРЕКЛЮЧАЮТ режим, а не устанавливают его (один код на
 * вкл/выкл — так же делают CURON/CUROFF в ПЗУ), поэтому шлём их только при
 * несовпадении с нужным состоянием: соответствующий бит ССД читается через
 * EMT 34. Так же поступает ПЗУ Бейсика (BK.mac, метка MIDM).
 *
 * Безусловная посылка 0233 ломала перезапуск заставки по клавише «СТОП»: БК уже
 * была в цветном режиме, и переключатель уводил дисплей в ч/б 64 символа в
 * строке — левая панель печаталась узким шрифтом.
 *
 * Курсор гасим ПОСЛЕ смены режима: смена режима заново включает и рисует курсор
 * в левом верхнем углу — этот вызов его стирает. Иначе курсор оставался бы
 * виден (в углу рамки и в конце текста).
 */
static void init_display()
{
    if (!EMT_34().bits.COLOR) EMT_16(0233);      // Цветной режим (32 символа в строке)
    EMT_16(0222);                                // Цвет вывода = зелёный (код-установщик)
    if (!EMT_34().bits.CURSOR_OFF) EMT_16(0232); // Погасить курсор
}
...
void main()
{
    ...
    (void)*(volatile uint16_t *)REG_KEY_DATA; // Очистка буфера клавиатуры от кода,
                                              // оставшегося до перезапуска
    ...
    reset_demo_state(); // Исходное состояние демо (переменные переживают перезапуск по «СТОП»)
