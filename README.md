# Порт игры Digger (Windmill Software Inc.) для компьютера БК-0010

![Digger](Digger.png)

Одной из целей проекта была возможность показать, что для создания программ на БК-0010 можно использовать
современный компилятор языка Си, такой, как gcc.
...
## Сборка проекта Digger под БК-0010

Используется система сборки **make**. При наличии установленного компилятора **pdp11-aout-gcc** для сборки проекта достаточно запустить утилиту **make** в папке с проектом:
```
make
```
...
Секция **asm-file** - отладочная. В ней из некоторый файлов Си гененрируются файлы на языке ассемблер (файлы с расширением **.s**).
...
Для сборки этой утилиты необходимо, чтобы был установлен компилятор **gcc** (обычный, не кросс).
