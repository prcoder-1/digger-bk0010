...
- **DIGTIT_V1.0.BIN** - заставка: распаковывает обложку в экранное ОЗУ (ZX0),
...
- **DIGGER_V1.0.BIN** - собственно игра: уровни, спрайты, звуковые эффекты и фоновая музыка
...
В нём находятся описания 8-ми и 16-ти битных целых типов данных необходимые для сборки проекта.
32-битные типы намеренно не описаны: на БК-0010 они дают заметный проигрыш по объёму кода и скорости,
поэтому в проекте не используются (счёт очков - 16-битный, с потолком 65535).
...
В результате работы **make** получаются два запускаемых файла: **DIGGER_V1.0.BIN** (игра) и **DIGTIT_V1.0.BIN** (заставка).
